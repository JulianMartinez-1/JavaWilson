...
public class vehiControlador extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if ("eliminarUno".equals(accion)) {
            String placavehi = request.getParameter("placavehi");
            if (placavehi != null && !placavehi.isEmpty()) {
                if (vehiDAO.eliminarvehiuno(placavehi)) {
                    request.setAttribute("mensaje", "Vehículo eliminado exitosamente");
                } else {
                    request.setAttribute("mensaje", "Error al eliminar el vehículo");
                }
            }
            response.sendRedirect("listarVehi.jsp");
        } else if ("editar".equals(accion)) {
            String placavehi = request.getParameter("placavehi");
            if (placavehi != null) {
                vehi vehiculo = vehiDAO.buscarVehiculo(placavehi);
                request.setAttribute("vehiculo", vehiculo);
                request.getRequestDispatcher("editarVehi.jsp").forward(request, response);
            }
        }
    }
